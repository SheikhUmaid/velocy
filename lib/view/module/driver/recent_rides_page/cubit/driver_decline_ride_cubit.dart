...
class DriverDeclineRideCubit extends Cubit<CommonState> {
  final DriverAuthRepository driverAuthRepository;

  DriverDeclineRideCubit({required this.driverAuthRepository}) : super(CommonInitial());

  Future<void> driverDeclineRide(int id) async {
    emit(CommonLoading());

    try {
      final response = await driverAuthRepository.declineRide(id);
