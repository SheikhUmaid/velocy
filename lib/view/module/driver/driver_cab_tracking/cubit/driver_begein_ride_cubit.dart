...
class DriverBeginRideCubit extends Cubit<CommonState> {
  final DriverAuthRepository driverAuthRepository;

  DriverBeginRideCubit({required this.driverAuthRepository}) : super(CommonInitial());

  Future<void> driverBeginRide(int id) async {
    emit(CommonLoading());

    try {
      final response = await driverAuthRepository.beginRide(id);
